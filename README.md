...
![System block diagram](docs/figures/architecture.svg)

The C209 BLE tag broadcasts BLE 5.1 advertisement packets containing a Constant Tone Extension (CTE). The ANT-B10 antenna board (8-element patch array with NINA-B411 module) captures IQ samples from each element, and the EVB-ANT-1 application board (NXP RT1061 MCU running u-locateEmbed firmware) computes the per-packet azimuth/elevation angle and streams the result over USB CDC serial. On the Stretch host, our `xplr_aoa3_node` ROS 2 node parses the serial stream and publishes to `/aoa/measurement` at ~20 Hz. The localization scripts (`process_bags.py`, `tag_localization*.py`) consume recorded bags and solve for tag position via least-squares ray intersection over six anchor poses.
